...
_check_sudoers() {
    ...
    # Quick test: can we sudo -u to any user AND pass DOCKER_HOST through?
    local test_user
    test_user="$(getent passwd 1101 2>/dev/null | cut -d: -f1 || true)"

    if [[ -n "$test_user" ]]; then
        # Test basic access
        if ! sudo -n -u "$test_user" true 2>/dev/null; then
            : # fall through to setup
        # Test env passthrough (SETENV required for dockeru to work)
        elif sudo -n -u "$test_user" env DOCKERU_TEST=1 sh -c 'test "$DOCKERU_TEST" = "1"' 2>/dev/null; then
            printf '%s✓ sudo access to daemon users appears to work (env passthrough verified).%s\n' "$GREEN" "$RESET"
            return
        else
            printf '%s⚠ sudo access works but environment variables are stripped (SETENV missing).%s\n' "$YELLOW" "$RESET"
            printf 'DockerU requires SETENV to pass DOCKER_HOST to daemon processes.\n\n'
        fi
    fi
    ...
    sudoers_content+="# SETENV is required so dockeru can pass DOCKER_HOST to the docker binary.\n\n"

    for du in "${daemon_users[@]}"; do
        sudoers_content+="${current_user} ALL=(${du}) NOPASSWD: SETENV: ${docker_bin}\n"
    done
